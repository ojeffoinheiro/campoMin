...
const spreadMines = (board, minesAmount) => {
    ...
    let minesPlantede = 0
    while(minesPlantede < minesAmount){
        const rowSel = parseInt(Math.random() * rows, 10)
        const colummnSel = parseInt(Math.random() * columns, 10)

        if(!board[rowSel][colummnSel].mined){
            board[rowSel][colummnSel].mined = true
            minesPlantede++
        }
    }
}
...
const hadExplosion = board => field(board).filter(field => field.exploded).length > 0
...
const flagUsed = board => field(board).filter(field => field.flagged).length
